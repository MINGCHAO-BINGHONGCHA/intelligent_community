...
class my_nanodet():
    def __init__(self, input_shape=320, prob_threshold=0.4, iou_threshold=0.3, model_path='nanodet.onnx', clsname_path='coco.names'):
    ...
    def get_bboxes_single(self, cls_scores, bbox_preds, scale_factor, rescale=False):
        ...
        # cv2.dnn.NMSBoxes 在不同 OpenCV 版本中返回形式可能不同（例如 list of [i], 或 ndarray shape (N,1)，或 []）
        indices = cv2.dnn.NMSBoxes(bboxes_wh.tolist(), confidences.tolist(), float(self.prob_threshold), float(self.iou_threshold))
        ...
        # 如果是形如 [[0],[2]] -> 扁平化；如果是 (N,1) -> 扁平化；如果已经是一维也可
        inds = inds.reshape(-1).astype(int)

        mlvl_bboxes = mlvl_bboxes[inds]
        confidences = confidences[inds]
        classIds = classIds[inds]
